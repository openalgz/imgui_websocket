...
struct ImGuiWS {
  ...
  bool init(int32_t port, std::string http_root,
            std::vector<std::string> resources);
  bool init(int32_t port, std::string http_root,
            std::vector<std::string> resources, THandler &&connect_handler,
            THandler &&disconnect_handler);
  bool setTexture(TextureId textureId, Texture::Type textureType, int32_t width,
                  int32_t height, const char *data);
